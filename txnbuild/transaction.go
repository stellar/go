...
// Hash returns the network specific hash of this transaction
// encoded as a byte array.
func (t *GenericTransaction) Hash(network string) ([32]byte, error) {
	if t.simple != nil {
		return t.simple.Hash(network)
	}
	return t.feeBump.Hash(network)
}
...
func (t *GenericTransaction) HashHex(network string) (string, error) {
	if t.simple != nil {
		return t.simple.HashHex(network)
	}
	return t.feeBump.HashHex(network)
}

// Sign returns a new FeeBumpTransaction instance which extends the current instance
// with a additional signatures derived from the given list of keypair instances.
func (t *GenericTransaction) Sign(network string, kps ...*keypair.Full) (*GenericTransaction, error) {
	var err error
	next := &GenericTransaction{}

	if t.simple != nil {
		next.simple, err = t.simple.Sign(network, kps...)
	} else {
		next.feeBump, err = t.feeBump.Sign(network, kps...)
	}
	return next, err
}
...
// SignWithKeyString returns a new FeeBumpTransaction instance which extends the current instance
// with a additional signatures derived from the given list of private key strings.
func (t *GenericTransaction) SignWithKeyString(network string, keys ...string) (*GenericTransaction, error) {
	var err error
	next := &GenericTransaction{}

	if t.simple != nil {
		next.simple, err = t.simple.SignWithKeyString(network, keys...)
	} else {
		next.feeBump, err = t.feeBump.SignWithKeyString(network, keys...)
	}
	return next, err
}

// Don't implement SignHashX for GenericTransaction because that action only
// makes sense for non fee bump transactions

// TxEnvelope returns the a xdr.TransactionEnvelope instance which is
// equivalent to this transaction.
func (t *GenericTransaction) TxEnvelope() (xdr.TransactionEnvelope, error) {
	if t.simple != nil {
		return t.simple.TxEnvelope()
	}
	return t.feeBump.TxEnvelope()
}

// MarshalBinary returns the binary XDR representation of the transaction envelope.
func (t *GenericTransaction) MarshalBinary() ([]byte, error) {
	if t.simple != nil {
		return t.simple.MarshalBinary()
	}
	return t.feeBump.MarshalBinary()
}

// Base64 returns the base 64 XDR representation of the transaction envelope.
func (t *GenericTransaction) Base64() (string, error) {
	if t.simple != nil {
		return t.simple.Base64()
	}
	return t.feeBump.Base64()
}
