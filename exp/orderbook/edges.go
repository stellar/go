...
type edgeSet map[string][]TradeOpportunity
...
func (e edgeSet) add(key string, offer xdr.OfferEntry) {
	...
	offers := e[key]
	if len(offers) == 0 {
		e[key] = []TradeOpportunity{NewOfferOpportunity(offer)}
		return
	}

	// find the smallest i such that Price of offers[i] >  Price of offer
	insertIndex := sort.Search(len(offers), func(i int) bool {
		return offer.Price.Cheaper(offers[i].GetPrice())
	})

	offers = append(offers, NewOfferOpportunity(offer))
	last := len(offers) - 1
...
func (e edgeSet) remove(offerID xdr.Int64, key string) bool {
	...
	for i := 0; i < len(edges); i++ {
		if edges[i].GetId() == uint64(offerID) {
			contains = true
