...
func (haa *HistoryArchiveAdapter) GetState(sequence uint32, tempStore io.StateReaderTempStore) (io.StateReader, error) {
	exists, err := haa.archive.CategoryCheckpointExists("history", sequence)
	if err != nil {
	...
	sr, e := io.MakeSingleLedgerStateReader(haa.archive, tempStore, sequence, msrBufferSize)
	if e != nil {
