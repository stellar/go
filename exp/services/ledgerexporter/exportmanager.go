...
func (f *LedgerCloseMetaObject) AddLedgerCloseMeta(ledgerCloseMeta xdr.LedgerCloseMeta) {
	...
	f.endLedger = ledgerCloseMeta.LedgerSequence()
}
...
func (e *ExportManager) AddLedgerCloseMeta(ledgerCloseMeta xdr.LedgerCloseMeta) error {
	...
	if !exists {
		...
		ledgerCloseMetaObject = &LedgerCloseMetaObject{startLedger: ledgerSeq}
		ledgerCloseMetaObject.data.StartSequence = xdr.Uint32(ledgerSeq)
		ledgerCloseMetaObject.objectKey = objectKey
	...
	if ledgerCloseMetaObject.LedgerCount() == e.config.LedgersPerFile {
