...
func createTransactionEnvelope() (xdr.TransactionEnvelope, error) {
	randomAccount := keypair.MustRandom().Address()
	src, err := xdr.AddressToAccountId(randomAccount)
	if err != nil {
...
func TestRestoreChangeType(t *testing.T) {
	...
	tx, err := createTransactionEnvelope()
	assert.NoError(t, err)
	tx1, err := createTransactionEnvelope()
	assert.NoError(t, err)
	tx2, err := createTransactionEnvelope()
	assert.NoError(t, err)
	tx3, err := createTransactionEnvelope()
	assert.NoError(t, err)
	txHash, err := network.HashTransactionInEnvelope(tx, network.TestNetworkPassphrase)
	assert.NoError(t, err)
	tx1Hash, err := network.HashTransactionInEnvelope(tx1, network.TestNetworkPassphrase)
	assert.NoError(t, err)
	tx2Hash, err := network.HashTransactionInEnvelope(tx2, network.TestNetworkPassphrase)
	assert.NoError(t, err)
	tx3Hash, err := network.HashTransactionInEnvelope(tx3, network.TestNetworkPassphrase)
	assert.NoError(t, err)
	...
	ledger := xdr.LedgerCloseMeta{
		...
		V1: &xdr.LedgerCloseMetaV1{
			...
			TxProcessing: []xdr.TransactionResultMeta{
				// contract data restore
				{
					Result: xdr.TransactionResultPair{TransactionHash: txHash},
					TxApplyProcessing: xdr.TransactionMeta{
						V: 3,
						V3: &xdr.TransactionMetaV3{
							Operations: []xdr.OperationMeta{
				...
				//contract code restore
				{
					Result: xdr.TransactionResultPair{TransactionHash: tx1Hash},
					TxApplyProcessing: xdr.TransactionMeta{
						V: 3,
						V3: &xdr.TransactionMetaV3{
							Operations: []xdr.OperationMeta{
				...
				// archived ttl restore
				{
					Result: xdr.TransactionResultPair{TransactionHash: tx2Hash},
					TxApplyProcessing: xdr.TransactionMeta{
						V: 3,
						V3: &xdr.TransactionMetaV3{
							Operations: []xdr.OperationMeta{
				...
				// evicted ttl restore
				{
					Result: xdr.TransactionResultPair{TransactionHash: tx3Hash},
					TxApplyProcessing: xdr.TransactionMeta{
						V: 3,
						V3: &xdr.TransactionMetaV3{
							Operations: []xdr.OperationMeta{
										...
										xdr.LedgerEntryChange{
											...
											Created: &xdr.LedgerEntry{
												Data: xdr.LedgerEntryData{
													Type: xdr.LedgerEntryTypeAccount,
													Account: &xdr.AccountEntry{
														AccountId: xdr.MustAddress(feeAddress),
	...
	require.NotNil(t, change.Post.Data.ContractData)
	...
	require.NotNil(t, change.Post.Data.ContractCode)
	...
	require.NotNil(t, change.Post.Data.Ttl)
