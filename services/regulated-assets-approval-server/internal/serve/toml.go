...
func (h stellarTOMLHandler) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(rw, "approval_criteria=\"The approval server currently only accepts payments. The transaction must have exactly one operation of type payment. If the payment amount exceeds %s %s it will need KYC approval if you haven’t been previously approved.\"", kycThreshold, h.assetCode)
}
