...
func (r *Router) addRoutes(config *RouterConfig, rateLimiter *throttled.HTTPRateLimiter, serverMetrics *ServerMetrics, ledgerState *ledger.State) {
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/liquidity_pools", func(r chi.Router) {
			...
			r.Route("/{liquidity_pool_id:\\w+}", func(r chi.Router) {
				...
				r.With(historyMiddleware).Method(http.MethodGet, "/trades", streamableHistoryPageHandler(ledgerState, responseAgeObserver, actions.GetTradesHandler{LedgerState: ledgerState, CoreStateGetter: config.CoreGetter}, streamHandler))
			})
		})
	...
	r.Group(func(r chi.Router) {
		...
		r.With(historyMiddleware).Method(http.MethodGet, "/accounts/{account_id:\\w+}/trades", streamableHistoryPageHandler(ledgerState, responseAgeObserver, actions.GetTradesHandler{LedgerState: ledgerState, CoreStateGetter: config.CoreGetter}, streamHandler))
	...
	r.Group(func(r chi.Router) {
		...
		// routes here.
		r.With(historyMiddleware).Method(http.MethodGet, "/offers/{offer_id}/trades", streamableHistoryPageHandler(ledgerState, responseAgeObserver, actions.GetTradesHandler{LedgerState: ledgerState, CoreStateGetter: config.CoreGetter}, streamHandler))
	})
