...
* Add experimental "Accounts For Signers" endpoint. To enable it set `--enable-accounts-for-signer` CLI param or `ENABLE_ACCOUNTS_FOR_SIGNER=true` env variable. This will exposes `/accounts` endpoint. This requires around 4GB of RAM for initial state ingestion.
