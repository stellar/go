...
func TestFlagsOutput(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:              "scanreplace sub-command with start and end present",
			commandArgs:       []string{"scan-and-replace", "--start", "10", "--end", "20", "--config-file", "myfile"},
			expectedErrOutput: "",
			appRunner:         appRunnerSuccess,
			expectedSettings: RuntimeSettings{
				StartLedger:    10,
		...
		{
			name:              "scanreplace sub-command with start and end absent",
			commandArgs:       []string{"scan-and-replace", "--config-file", "myfile"},
			expectedErrOutput: "",
			appRunner:         appRunnerSuccess,
			expectedSettings: RuntimeSettings{
				StartLedger:    0,
		...
		{
			name:              "scanreplace sub-command prints app error",
			commandArgs:       []string{"scan-and-replace", "--start", "10", "--end", "20", "--config-file", "myfile"},
			expectedErrOutput: "test error",
			appRunner:         appRunnerError,
		},
